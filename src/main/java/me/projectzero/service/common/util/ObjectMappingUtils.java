...
public class ObjectMappingUtils {
     ...
     */
    public <T extends IdentifiableEntity<I>, I> T getEntityWithId(@NonNull Supplier<T> newEntity, @Nullable I entityId, T oldEntity) {
        return entityId != null ? IdentifiableEntity.getInstance(newEntity, entityId) : oldEntity;
    }
     ...
     */
    public <T extends IdentifiableEntity<I>, I, C extends Collection<T>> C getEntitiesWithIds(@NonNull Supplier<C> newCollection, Supplier<T> newEntity, Collection<I> entityIdList) {
        if (entityIdList == null || newEntity == null) return newCollection.get();
        C entities = newCollection.get();
        for (I id : entityIdList) entities.add(IdentifiableEntity.getInstance(newEntity, id));
        return entities;
    }
